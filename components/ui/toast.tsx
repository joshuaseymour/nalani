...
const Toast = React.forwardRef<
  HTMLDivElement,
  React.HTMLAttributes<HTMLDivElement> & VariantProps<typeof toastVariants> & { toastId: string }
>(({ className, variant, toastId, ...props }, ref) => {
  const context = React.useContext(ToastContext)
  if (!context) throw new Error("Toast must be used within ToastProvider")

  return <div ref={ref} className={cn(toastVariants({ variant }), className)} data-state="open" {...props} />
})
...
>(({ className, toastId, ...props }, ref) => {
  const context = React.useContext(ToastContext)
  if (!context) throw new Error("ToastClose must be used within ToastProvider")
  ...
  return (
    <button
      ...
      onClick={() => context.removeToast(toastId)}
      toast-close=""
      {...props}
    >
      <X className="h-4 w-4" />
...
export { ToastProvider, ToastViewport, type Toast, ToastTitle, ToastDescription, ToastClose, ToastAction }
